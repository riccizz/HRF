...
class ExactOptimalTransportConditionalFlowMatcher(ConditionalFlowMatcher):
    ...
    def __init__(self, sigma: Union[float, int] = 0.0, ot_bs: int = 0):
        r"""Initialize the ConditionalFlowMatcher class. It requires the hyper-parameter $\sigma$.
    ...
    def sample_location_and_conditional_flow(self, x0, x1, t=None, return_noise=False):
        ...
        [1] Improving and Generalizing Flow-Based Generative Models with minibatch optimal transport, Preprint, Tong et al.
        """
        x0, x1 = self.batchOT(x0, x1)
        return super().sample_location_and_conditional_flow(x0, x1, t, return_noise)

    def batchOT(self, x0, x1):
        bs = x0.shape[0]
        if self.ot_bs == 0 or self.ot_bs >= bs:
            x0, x1 = self.ot_sampler.sample_plan(x0, x1)
            return x0, x1
        else:
            ot_bs = self.ot_bs
            ot_x0 = torch.zeros_like(x0)
            ot_x1 = torch.zeros_like(x1)
            for i in range(int(bs // ot_bs)):
                start = i * ot_bs
                end = min((i+1) * ot_bs, bs)
                ot_x0[start:end], ot_x1[start:end] = self.ot_sampler.sample_plan(x0[start:end], x1[start:end])
            return ot_x0, ot_x1
